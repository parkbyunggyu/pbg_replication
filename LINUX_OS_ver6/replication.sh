...
function hbabye()
{
	...
	sed -i ''$UT'd' $TEMP3/pg_hba.conf
	sed -i ''$UT'd' $TEMP3/pg_hba.conf
	scp $TEMP3/pg_hba.conf root@$IP:$MST_DATA_DIR/ &>/dev/null
...
function arch()
{
	...
	do
		...
		echo "Copy WAL file that created after pg_basebackup end, to restore directory" &>>$LOG_FILE
	        scp root@$IP:$MST_ARCH_DIR/0000* $SLV_ARCH_DIR/  &>>$LOG_FILE
...
if [ "$B" == "y" ]; then
        ...
        scp root@$1:$TEMP/.bash_profile $TEMP &>/dev/null
        if [ "$P1" == "y" ]; then
...
scp $TEMP/.bash_profile root@$1:$TEMP &>/dev/null
...
mkdir $TEMP3
scp root@$1:$MST_DATA_DIR/pg_hba.conf $TEMP3 &>/dev/null
...
EOFF
scp $TEMP3/pg_hba.conf root@$1:$MST_DATA_DIR/ &>/dev/null
...
if [ "$W" == "d" ]; then
	...
	WS=1
        ...
        MST_WAL_DIR=`ssh -T -p $SSHPORT root@$1 su - $MST_SVC_USR -c \"ls -ld $MST_DATA_DIR/pg_xlog\" | awk '{print $10}'`
...
if [ "$C" == "on" ]; then
        if [ "$L" == "pg_log" ]; then
                ...
                if [ "$LL" == "d" ]; then
                        ...
                        MST_LOG_DIR=`ssh -T -p $SSHPORT root@$1 su - $MST_SVC_USR -c \"ls -ld $MST_DATA_DIR/pg_log\" | awk '{print $10}'`
...
if [ "$BACKUP" == "y" ];then
	...
	su - $SLV_SVC_USR -c "`ls -l $OSLV_DATA_DIR/pg_tblspc/|awk '{print "cp -a "$10" '$BACKUP_DIR'/TBS/"}' | tail -n +2`" &>/dev/null
	if [ $? -eq 0 ]; then
		su - $SLV_SVC_USR -c "`ls -l $OSLV_DATA_DIR/pg_tblspc/|awk '{print "rm -rf "$10}' | tail -n +2`" &>/dev/null
	fi 
...
if [ "$MSTSLVEQ" == "y" ]; then
	YAHO=`ssh -T -p $SSHPORT root@$1 ls -l $MST_DATA_DIR/pg_tblspc/|awk '{print "-T "$10"="$10}' | tail -n +2`
	TEMP=/tmp/temp$(date +%Y%m%d%H%M%S).sh
	ssh -T -p $SSHPORT root@$1 ls -l $MST_DATA_DIR/pg_tblspc/|awk '{print "rm -rf "$10}' | tail -n +2 >> $TEMP
	ssh -T -p $SSHPORT root@$1 ls -l $MST_DATA_DIR/pg_tblspc/|awk '{print "mkdir -p "$10}' | tail -n +2 >> $TEMP
	ssh -T -p $SSHPORT root@$1 ls -l $MST_DATA_DIR/pg_tblspc/|awk '{print "chown -R '$SLV_SVC_USR'. "$10}' | tail -n +2 >> $TEMP
	bash $TEMP
	rm -rf $TEMP
	YAHO=`echo $YAHO`
elif [ "$MSTSLVEQ" == "n" ]; then
	...
	touch $TEMP
cat > $TEMP <<EOFF
`ssh -T -p $SSHPORT root@$1 ls -l $MST_DATA_DIR/pg_tblspc/|awk '{print "p"$8"="$10}' | tail -n +2`
...
Y=`ssh -T -p $SSHPORT root@$1 su - $MST_SVC_USR -c \"find $MST_ENGN_HOME -name recovery.conf.sample\"`
...
su - $SLV_SVC_USR -c "pg_basebackup $YAHO -h $1 -p $MST_PORT -P -D $SLV_DATA_DIR -X stream $XLOGDIR -U $MST_REP_USR"
ET=`date +%s`
ETS=`date` 
EST=`echo "$ET - $ST" | bc`
HT=`echo "$EST/3600" | bc` 
...
R=`ssh -T -p $SSHPORT root@$1 ls -rtl $MST_ARCH_DIR/*backup | tail -n 1 | awk '{print $8}'`
R=`ssh -T -p $SSHPORT root@$1 cat $R | head -n 2 | tail -n 1 |rev|cut -d' ' -f1 |rev`
R=`echo ${R%)*}`
