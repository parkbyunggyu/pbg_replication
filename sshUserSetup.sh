...
firstport=`echo $HOSTS | awk '{print $1}; END { }' |awk -F ':' '{print $2}'`
firsthost=`echo $HOSTS | awk '{print $1}; END { }' |awk -F ':' '{print $1}'`
...
do
     ...
     host="-p $port $host"
	 $SSH -o StrictHostKeyChecking=no -x -l $USR $host "/bin/sh -c \"  mkdir -p .ssh ; chmod og-w . .ssh;   touch .ssh/authorized_keys .ssh/known_hosts;  chmod 644 .ssh/authorized_keys  .ssh/known_hosts; cp  .ssh/authorized_keys .ssh/authorized_keys.tmp ;  cp .ssh/known_hosts .ssh/known_hosts.tmp; echo \\"Host *\\" > .ssh/config.tmp; echo \\"ForwardX11 no\\" >> .ssh/config.tmp; if test -f  .ssh/config ; then cp -f .ssh/config .ssh/config.backup; chmod 644 .ssh/config; fi ; mv -f .ssh/config.tmp .ssh/config; chmod 644 .ssh/config\""  | tee -a $LOGFILE
...
do
  ...
  $SCP -P $port $HOME/.ssh/${IDENTITY}.pub  $USR@$host:.ssh/authorized_keys | tee -a $LOGFILE
...
do
  ...
  then
      $SCP -P $port $USR@$host:.ssh/${IDENTITY}.pub $HOME/.ssh/${IDENTITY}.pub.$host | tee -a $LOGFILE
...
do
   ...
   then
      ...
      then
         ...
         $SCP -P $port $HOME/.ssh/authorized_keys  $USR@$host:.ssh/authorized_keys | tee -a $LOGFILE
      fi 
     echo Updating known_hosts file on remote host $host | tee -a $LOGFILE
     $SCP -P $port $HOME/.ssh/known_hosts $USR@$host:.ssh/known_hosts | tee -a $LOGFILE
   fi
   if [ $PASSPHRASE = "yes" ]
   then
...
then
   ...
   do
      portc=`echo $clienthost|awk -F ':' '{print $2}'`
      clienthost=`echo $clienthost|awk -F ':' '{print $1}'`
      if [ $SHARED = "true" ]
      then
      ...
      do
	     ports=`echo $serverhost|awk -F ':' '{print $2}'`
		 serverhost=`echo $serverhost|awk -F ':' '{print $1}'`
		 ...
		 clienthostt="-p $portc $clienthost"
         $SSH -l $USR $clienthostt "$REMOTESSH $serverhost \"/bin/sh -c date\""  | tee -a $LOGFILE
         echo ------------------------------------------------------------------------ | tee -a $LOGFILE
      done  
   ...
   then
     ...
     do
        ...
        echo "IF YOU SEE ANY OTHER OUTPUT BESIDES THE OUTPUT OF THE DATE COMMAND OR IF YOU ARE PROMPTED FOR A PASSWORD HERE, IT MEANS SSH SETUP HAS NOT BEEN SUCCESSFUL." | tee -a $LOGFILE
		firsthostt="-p $firstport $firsthost"
       $SSH -l $USR $firsthostt "/bin/sh -c \"$REMOTESSH $host \\"/bin/sh -c date\\"\"" | tee -a $LOGFILE
         echo ------------------------------------------------------------------------ | tee -a $LOGFILE
    done
    echo -Verification from $clienthost complete- | tee -a $LOGFILE
  fi
